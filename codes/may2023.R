...
May2023$hour <- hour(May2023$started_at)

#hourly usage for members
hourly_usage_member <- May2023 %>%
...
ggplot(data = hourly_usage_member)+
  ...
  labs(x = "Hour of the Day", y = "Number of Bike Usage During Each Hour", title = "Usage of Bikes Throughout the Day by Members May 2023")+
  annotate("text", x = 8, y = 27000, label = "Peak at 8AM")+
  annotate("text", x = 17, y = 43000, label = "Peak at 5PM")

#hourly usage for casuals
hourly_usage_casual <- May2023 %>%
...
ggplot(data = hourly_usage_casual)+
  geom_line(mapping = aes(x = hour, y = count))+
  scale_x_continuous(breaks = seq(0,23, by = 1))+
  labs(x = "Hour of the Day", y = "Number of Bike Usage During Each Hour", title = "Usage of Bikes Throughout the Day by Casuals May 2023")+
  annotate("text", x = 17, y = 27500, label = "Peak at 5PM")
...
qmplot(end_lng, end_lat, data = five_pm_coordinates_member, maptype = "stamen_terrain", alpha = "ended_at")+
  labs(title = "Member's End location at 5PM May 2023")+
...
qmplot(end_lng, end_lat, data = five_pm_coordinates_casual, maptype = "stamen_terrain", alpha = "ended_at")+
  labs(title = "Casual's End location at 5PM May 2023")+
  theme(legend.title = element_blank(), legend.text = element_blank(), legend.key = element_blank())

#filter coordinates at 8am
eight_am_coordinates <- May2023 %>%
  filter(hour == "8") %>%
  group_by(end_lat, end_lng, hour)
...
qmplot(end_lng, end_lat, data = eight_am_coordinates_member, maptype = "stamen_terrain", alpha = "ended_at")+
  labs(title = "Member's End location at 8AM May 2023")+
  theme(legend.title = element_blank(), legend.text = element_blank(), legend.key = element_blank())
...
ggplot(data = member_summary)+
  ...
  annotate("text", x = 15, y = 17500, label = "Memorial Day 5/29")+
  labs(title = "Amount of Member Riders Throughout May 2023", y = "Number of Riders", x = "Date")
...
ggplot(data = casual_summary)+
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 0))+
  annotate("text", x = 15, y = 15500, label = "Memorial Day 5/29")+
  labs(title = "Amount of Casual Riders Throughout May 2023", y = "Number of Riders", x = "Date")
...
May2023$started_at <- as.Date(May2023$started_at, format = "%m-%d-%y")
May2023$day_of_week <- weekdays(May2023$started_at)
day_order <- c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")
May2023$day_of_week <-fct_relevel(May2023$day_of_week, day_order)


#keep track of the records for all users
day_counts <- table(May2023$day_of_week)
day_counts_df <- as.data.frame(day_counts)
colnames(day_counts_df) <- c("day_of_week", "count")

#keep track of the records for members
member_records_dates <- May2023 %>%
  filter(member_casual == "member")%>%
  group_by(day_of_week, member_casual) %>%
  summarize(count = n())

#keep track of the records for casuals
casual_records_dates <- May2023 %>%
  filter(member_casual == "casual")%>%
  group_by(day_of_week, member_casual) %>%
  summarize(count = n())
...
ggplot(data = May2023)+
  ...
  labs(x = "Days of the Week", y = "Count", title = "Counts of Each Day of the Week May 2023", fill = "Type of Member")
...
ggplot(data = member_records_dates)+
  ...
  labs(x = "Days of the Week", y = "Count", title = "Counts of Each Day of the Week for Members May 2023")
...
ggplot(data = casual_records_dates)+
  ...
  labs(x = "Days of the Week", y = "Count", title = "Counts of Each Day of the Week for Casuals May 2023")
