...
Feb2024$hour <- hour(Feb2024$started_at)

#hourly usage for members
hourly_usage_member <- Feb2024 %>%
...
ggplot(data = hourly_usage_member)+
  ...
  labs(x = "Hour of the Day", y = "Number of Bike Usage During Each Hour", title = "Usage of Bikes Throughout the Day by Members February 2024")+
  annotate("text", x = 8, y = 15000, label = "Peak at 8AM")+
  annotate("text", x = 17, y = 21000, label = "Peak at 5PM")

#hourly usage for casuals
hourly_usage_casual <- Feb2024 %>%
...
ggplot(data = hourly_usage_casual)+
  geom_line(mapping = aes(x = hour, y = count))+
  scale_x_continuous(breaks = seq(0,23, by = 1))+
  labs(x = "Hour of the Day", y = "Number of Bike Usage During Each Hour", title = "Usage of Bikes Throughout the Day by Casuals February 2024")+
  annotate("text", x = 8, y = 2500, label = "Peak at 8AM")+
  annotate("text", x = 16, y = 5500, label = "Peak at 4PM")
...
qmplot(end_lng, end_lat, data = five_pm_coordinates_member, maptype = "stamen_terrain", alpha = "ended_at")+
  labs(title = "Member's End location at 5PM February 2024")+
  theme(legend.title = element_blank(), legend.text = element_blank(), legend.key = element_blank())

#filter coordinates at 8am
eight_am_coordinates <- Feb2024 %>%
  filter(hour == "8") %>%
  group_by(end_lat, end_lng, hour)
...
qmplot(end_lng, end_lat, data = eight_am_coordinates_member, maptype = "stamen_terrain", alpha = "ended_at")+
  labs(title = "Member's End location at 8AM February 2024")+
  theme(legend.title = element_blank(), legend.text = element_blank(), legend.key = element_blank())
...
ggplot(data = member_summary)+
  ...
  annotate("text", x = 9, y = 10500, label = "02-09-24")+
  annotate("text", x = 14, y = 9500, label = "02-14-24")+
  annotate("text", x = 21, y = 13000, label = "02-21-24")+
  annotate("text", x = 27, y = 14000, label = "02-27-24")+
  labs(title = "Amount of Member Riders Throughout February 2024", y = "Number of Riders", x = "Date")
...
ggplot(data = casual_summary)+
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 0))+
  annotate("text", x = 9, y = 3000, label = "02-09-24")+
  annotate("text", x = 14, y = 2000, label = "02-14-24")+
  annotate("text", x = 21, y = 3000, label = "02-21-24")+
  annotate("text", x = 27, y = 4000, label = "02-27-24")+
  labs(title = "Amount of Casual Riders Throughout February 2024", y = "Number of Riders", x = "Date")
...
Feb2024$started_at <- as.Date(Feb2024$started_at, format = "%m-%d-%y")
Feb2024$day_of_week <- weekdays(Feb2024$started_at)
day_order <- c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")
Feb2024$day_of_week <-fct_relevel(Feb2024$day_of_week, day_order)


#keep track of the records for all users
day_counts <- table(Feb2024$day_of_week)
day_counts_df <- as.data.frame(day_counts)
colnames(day_counts_df) <- c("day_of_week", "count")

#keep track of the records for members
member_records_dates <- Feb2024 %>%
  filter(member_casual == "member")%>%
  group_by(day_of_week, member_casual) %>%
  summarize(count = n())

#keep track of the records for casuals
casual_records_dates <- Feb2024 %>%
  filter(member_casual == "casual")%>%
  group_by(day_of_week, member_casual) %>%
  summarize(count = n())
...
ggplot(data = Feb2024)+
  ...
  labs(x = "Days of the Week", y = "Count", title = "Counts of Each Day of the Week in February 2024", fill = "Type of Member")
...
ggplot(data = member_records_dates)+
  ...
  labs(x = "Days of the Week", y = "Count", title = "Counts of Each Day of the Week for Members February 2024")
...
ggplot(data = casual_records_dates)+
  ...
  labs(x = "Days of the Week", y = "Count", title = "Counts of Each Day of the Week for Casuals February 2024")
