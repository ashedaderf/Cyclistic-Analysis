...
July2023 <- read_csv("202307-divvy-tripdata.csv")
...
July2023$hour <- hour(July2023$started_at)

#hourly usage for members
hourly_usage_member <- July2023 %>%
...
ggplot(data = hourly_usage_member)+
  ...
  labs(x = "Hour of the Day", y = "Number of Bike Usage During Each Hour", title = "Usage of Bikes Throughout the Day by Members July 2023")+
  annotate("text", x = 8, y = 28000, label = "Peak at 8AM")+
  annotate("text", x = 17, y = 47000, label = "Peak at 5PM")

#hourly usage for casuals
hourly_usage_casual <- July2023 %>%
...
ggplot(data = hourly_usage_casual)+
  geom_line(mapping = aes(x = hour, y = count))+
  scale_x_continuous(breaks = seq(0,23, by = 1))+
  labs(x = "Hour of the Day", y = "Number of Bike Usage During Each Hour", title = "Usage of Bikes Throughout the Day by Casuals July 2023")+
  annotate("text", x = 8, y = 13000, label = "Peak at 8AM")+
  annotate("text", x = 12, y = 23500, label = "Peak at 12PM")+
  annotate("text", x = 17, y = 32500, label = "Peak at 5PM")
...
qmplot(end_lng, end_lat, data = five_pm_coordinates_member, maptype = "stamen_terrain", alpha = "ended_at")+
  labs(title = "Member's End location at 5PM July 2023")+
...
qmplot(end_lng, end_lat, data = five_pm_coordinates_casual, maptype = "stamen_terrain", alpha = "ended_at")+
  labs(title = "Casual's End location at 5PM July 2023")+
  theme(legend.title = element_blank(), legend.text = element_blank(), legend.key = element_blank())

#filter coordinates at 8am
eight_am_coordinates <- July2023 %>%
  filter(hour == "8") %>%
  group_by(end_lat, end_lng, hour)
...
qmplot(end_lng, end_lat, data = eight_am_coordinates_member, maptype = "stamen_terrain", alpha = "ended_at")+
  labs(title = "Member's End location at 8AM July 2023")+
  theme(legend.title = element_blank(), legend.text = element_blank(), legend.key = element_blank())
...
qmplot(end_lng, end_lat, data = eight_am_coordinates_casual, maptype = "stamen_terrain", alpha = "ended_at")+
  labs(title = "Casual's End location at 8AM July 2023")+
  theme(legend.title = element_blank(), legend.text = element_blank(), legend.key = element_blank())

#twelve pm casuals
twelve_pm_coordinates <- July2023 %>%
  filter(hour == "17") %>%
  group_by(end_lat, end_lng, hour)

twelve_pm_coordinates_casual <- twelve_pm_coordinates %>%
  filter(member_casual == "casual") %>%
  group_by(end_lat, end_lng, hour)

qmplot(end_lng, end_lat, data = twelve_pm_coordinates_casual, maptype = "stamen_terrain", alpha = "ended_at")+
  labs(title = "Casual's End location at 12PM July 2023")+
  theme(legend.title = element_blank(), legend.text = element_blank(), legend.key = element_blank())


#changing all the dates to just dates so we can do a histogram and see the time for just member or casual later.
July2023$started_at <- format(July2023$started_at, "%m-%d-%y")


#separated members and casuals so we can determine count for each individual one (MEMBER)

member_summary <- July2023 %>%
  group_by(started_at, member_casual)%>%
  filter(member_casual=="member")%>%
  summarize(count= n())

#separated members and casuals so we can determine count for each individual one (CASUAL)
casual_summary <- July2023 %>%
  group_by(started_at, member_casual)%>%
  filter(member_casual=="casual")%>%
  summarize(count = n())
...
ggplot(data = member_summary)+
  ...
  annotate("text", x = 15, y = 18000, label = "Independence Day 7/4")+
  labs(title = "Amount of Member Riders Throughout July 2023", y = "Number of Riders", x = "Date")
...
ggplot(data = casual_summary)+
  ...
  theme(axis.text.x = element_text(angle = 90, hjust = 0))+
  annotate("text", x = 15, y = 18000, label = "Independence Day 7/4")+
  labs(title = "Amount of Casual Riders Throughout July 2023", y = "Number of Riders", x = "Date")
...
July2023$started_at <- as.Date(July2023$started_at, format = "%m-%d-%y")
July2023$day_of_week <- weekdays(July2023$started_at)
day_order <- c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday")
July2023$day_of_week <-fct_relevel(July2023$day_of_week, day_order)


#keep track of the records for all users
day_counts <- table(July2023$day_of_week)
day_counts_df <- as.data.frame(day_counts)
colnames(day_counts_df) <- c("day_of_week", "count")

#keep track of the records for members
member_records_dates <- July2023 %>%
  filter(member_casual == "member")%>%
  group_by(day_of_week, member_casual) %>%
  summarize(count = n())

#keep track of the records for casuals
casual_records_dates <- July2023 %>%
  filter(member_casual == "casual")%>%
  group_by(day_of_week, member_casual) %>%
  summarize(count = n())
...
ggplot(data = July2023)+
  geom_bar(mapping=aes(x = day_of_week, fill=member_casual))+
  labs(x = "Days of the Week", y = "Count", title = "Counts of Each Day of the Week July 2023", fill = "Type of Member")
...
ggplot(data = member_records_dates)+
  ...
  labs(x = "Days of the Week", y = "Count", title = "Counts of Each Day of the Week for Members July 2023")
...
ggplot(data = casual_records_dates)+
  ...
  labs(x = "Days of the Week", y = "Count", title = "Counts of Each Day of the Week for Casuals July 2023")
